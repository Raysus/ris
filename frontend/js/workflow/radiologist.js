...
function risHtmlRetornoTranscripcionRadiologo(app) {
    ...
    return `
        <div class="alert alert-warning py-1 px-2 mb-2 small shadow-sm d-flex align-items-center gap-2 border-warning">
            <i class="bi bi-exclamation-octagon-fill"></i>
            <div><strong>Audio devuelto:</strong> ${risEscapeHtml(app.returnReason)}</div>
        </div>
    `;
}
...
function risHtmlAnamnesisRadiologo(app) {
    ...
    const clase = vacio ? 'ris-radio-anamnesis is-empty' : 'ris-radio-anamnesis';
    return `
        <div class="${clase}" title="${risEscapeHtml(texto)}">
            <span class="ris-radio-anamnesis-label"><i class="bi bi-chat-left-text" aria-hidden="true"></i> Anamnesis</span>
            <span class="ris-radio-anamnesis-text">${risEscapeHtml(texto)}</span>
        </div>
    `;
}
...
function renderEncabezadoPacienteRadiologo(app) {
    ...
    return `
        ...
        <div class="ris-radio-patient-header">
            <div class="ris-radio-patient-main">
                <div class="ris-radio-patient-name" title="${risEscapeHtml(nombreCompleto)}">
                    <i class="bi bi-person-bounding-box text-secondary" aria-hidden="true"></i>
                    <span>${risEscapeHtml(nombreCompleto)}</span>
                </div>
                <div class="ris-radio-patient-meta">
                    <span class="badge bg-dark font-monospace">${risEscapeHtml(app.accessionNumber)}</span>
                    <span class="text-muted"><b>RUT:</b> ${risEscapeHtml(app.patient.rut)}</span>
                </div>
            </div>
...
function limpiarPantallaRadiologo() {
    ...
    if (autoSaveInterval) clearInterval(autoSaveInterval);
    $("#infoPacienteRadiologo").html('<div class="text-center text-muted py-3"><i class="bi bi-file-earmark-medical fs-3 d-block mb-2"></i>Seleccione paciente.</div>');
