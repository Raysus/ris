#!/usr/bin/env python3
"""Inspecciona StudyController en portal-nuevo."""
import re
...
DASH = "/home/debuser/infra/portal-nuevo/resources/views/dashboard.blade.php"
...
def main() -> None:
    pwd = load_password()
    for path in (CTRL, DASH):
        print(f"\n===== {path} (grep RIS) =====")
        print(
            ssh(
                f"grep -n 'buildRisReportMap\\|resolveRisStudyId\\|ris_study_id\\|rut_hash\\|Informe RIS' {path}",
                pwd,
            )
        )
    print("\n===== showReport snippet =====")
    print(ssh(f"sed -n '/function showReport/,/^    }}/p' {CTRL} | head -55", pwd))
